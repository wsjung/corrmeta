...
```
This loads `trt1`, `trt2`, and `trt3` which are short, simulated SNP-trait association datasets. Note that although the examples are working on SNP datasets, `r Biocpkg("corrmeta")` works for any common OMIC unit of inference across each input dataset. `r Biocpkg("corrmeta")` requires that the input is a single dataframe where the OMIC units of inference are under column `markname` and each scan has its own column.
